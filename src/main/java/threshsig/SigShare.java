...
/**
 * Signature share: id, sig, and verifier.
 */
public class SigShare {
  ...
  public BigInteger getSig() { return sig; }
  public Verifier getSigVerifier() { return sigVerifier; }
  public byte[] getBytes() { return sig.toByteArray(); }
  ...
  public static boolean verify(final byte[] data, final SigShare[] sigs, final int k, final int l,
    ...
    for (int i = 0; i < k; i++) {
      if (sigs[i] == null) throw new ThresholdSigException("Null signature");
      if (haveSig[sigs[i].getId() - 1]) throw new ThresholdSigException("Duplicate signature: " + sigs[i].getId());
    ...
    if (CHECKVERIFIER) {
      ...
      try {
        ...
        for (int i = 0; i < k; i++) {
          ...
          if (!result.equals(ver.getC())) return false;
        }
      } catch (final java.security.NoSuchAlgorithmException ex) {
        throw new RuntimeException("SHA not found", ex);
      }
    }
    BigInteger w = BigInteger.valueOf(1L);
    for (int i = 0; i < k; i++) {
      w = w.multiply(sigs[i].getSig().modPow(lambda(sigs[i].getId(), sigs, delta), n));
    }
  ...
  private static BigInteger lambda(final int ik, final SigShare[] S, final BigInteger delta) {
    ...
    for (final SigShare element : S) {
      if (element.getId() != ik) value = value.multiply(BigInteger.valueOf(element.getId()));
    }
    for (final SigShare element : S) {
      if (element.getId() != ik) value = value.divide(BigInteger.valueOf((element.getId() - ik)));
    }
    return value;
  }
}
