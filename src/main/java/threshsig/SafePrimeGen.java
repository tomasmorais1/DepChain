...
class SafePrimeGen {
  ...
  public static BigInteger generateStrongPrime(final int bitLength, final SecureRandom random) {
    ...
    outerloop: do {
      ...
      c = s.modPow(b, r);
      d = c.multiply(a);
      BigInteger p0 = d.subtract(ThreshUtil.ONE);
      ...
      if (!b.mod(c).equals(ThreshUtil.ZERO)) j0 = j0.add(BigInteger.ONE);
      BigInteger j = j0;
      do {
        p = a.multiply(j).add(p0);
        j = j.add(BigInteger.ONE);
        if (p.bitLength() > bitLength) continue outerloop;
